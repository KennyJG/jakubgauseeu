<template>
    <div class="h-full mx-3 py-6">
        <div v-if="layout.type == 'ROW'">
            <div v-if="layout.childs.length == 0" class="mt-3 mb-3 text-gray-800 text-center">
                <div @click="createColumn()" class="cursor-pointer">
                    ...
                    <div class="mb-6 text-gray-500">+ Přidat sloupec</div>
                </div>
                <hr>
                <div class="mt-10">
            ...
            <div v-else>
                ...
                <div v-else>
                    ...
                    <div class="w-full bg-gray-100 border-dashed border-gray-800 border flex">
                        <div @click="setLayout(child.id); highlightSelected('Layout-' + child.id)" v-for="(child, index) in layout.childs" :style="'width:' + 100 / layout.childs.length + '%'" :class="{'border-r': index !== layout.childs.length - 1 }" class="h-32 border-gray-700 flex justify-center items-center bg-gray-300 hover:bg-gray-300 cursor-pointer">
                            <div>{{ Math.round(100 / layout.childs.length) + '%' }}</div>
                        </div>
                    </div>
