<template>
    <div class="text-sm bg-gray-300">
            ...
            <div class="pl-4">
                <div class="flex">
                    <div class="w-full px-1 flex items-center  hidden">
                        <div class="w-1/4 text-black mr-2">Název</div>
                        <div v-if="!edit" @click="startEditing()">
                            <span class="italic text-black" v-if="layout.name">{{ layout.name }}</span>
                            <span v-else class="italic text-gray-600">Bez názvu</span>
                        </div>
                        <div v-else class="flex">
                            <div class="w-full max-w-sm">
                                <div class="flex items-center border-b border-b-2 border-cyan-800">
                                    ...
                                    <button @click="save()" class="flex-shrink-0 border-transparent border-4 text-black hover:text-gray-600 text-sm rounded" type="button" style="outline: none;">Save</button>
                                    <button @click="stopEditing()" class="flex-shrink-0 border-transparent border-4 text-black hover:text-gray-600 text-sm rounded" type="button" style="outline: none;">Cancel</button>
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        <div class="bg-white border-t border-black relative mt-6">
            ...
            <layout-childrens class="mt-4" v-if="activeTab === 'CHILDRENS'"></layout-childrens>
            <layout-siblings class="mt-4" v-if="activeTab === 'SIBLINGS'"></layout-siblings>
        
            <div class="mt-4" v-if="activeTab == 'MAIN'">
                <section-styles v-if="layout.type == 'SECTION'"></section-styles>
                <column-styles v-if="layout.type == 'COLUMN'"></column-styles>
                <row-styles v-if="layout.type == 'ROW'"></row-styles>
                <element-styles v-if="layout.type == 'ELEMENT'"></element-styles>
            </div>
        </div>
    </div>
</template>
...
export default {
    ...
    computed: {
        ...mapGetters(['pages', 'layout', 'currentPageId'])
    },
    ...
    methods: {
        ...
        deleteLayout() {
            if (confirm('Opravdu chcete odstranit tento layout?')) {
                axios.delete('/api/layouts/' + this.layout.id)
                    .then((response) => {
                        this.getRenderedHtml(this.currentPageId);
                        this.unsetAll();
                    });
